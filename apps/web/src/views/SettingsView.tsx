...
type SettingsTab = 'api-keys' | 'audit' | 'notifications' | 'roles' | 'export' | 'graph'
...
export function SettingsView() {
  ...
  const tabs: Array<{ id: SettingsTab; label: string; description: string }> = [
    { id: 'api-keys', label: 'API Keys', description: 'Connect your agents via API' },
...
function ApiKeysPanel() {
  ...
  if (loading) return <p className="settings-loading">Loading API keys...</p>
    ...
    <div className="api-keys-panel">
      <div className="panel-header compact">
        <div>
          ...
          <h2>API Keys</h2>
          <p>Generate API keys to connect your AI agents to Copilot Flight Recorder.</p>
        </div>
      </div>
      ...
      {newKey && (
        <div className="api-key-new">
          ...
          <p className="api-key-warning">⚠ Store this key securely. It provides full access to your tenant's data ingestion.</p>
        </div>
      )}

      <div className="api-keys-list">
        {keys.length === 0 && <p style={{ color: '#5d6e88', padding: '16px 0' }}>No API keys yet. Create one to start integrating your agents.</p>}
        {keys.map((key) => (
          <div key={key.id} className={`api-key-row ${key.revoked ? 'revoked' : ''}`}>
            <div className="api-key-info">
              <span className="api-key-prefix">{key.prefix}•••••••••</span>
            ...
            {!key.revoked && (
              <button type="button" className="action-button subtle" onClick={() => handleRevoke(key.id)}>Revoke</button>
            )}
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-end' }}>
        <label style={{ flex: 1 }}>
          <span style={{ fontSize: '0.75rem', color: '#5d6e88' }}>Key name</span>
          <input
            type="text"
            value={keyName}
        ...
        <button type="button" className="action-button primary" onClick={handleCreate} disabled={creating}>
          {creating ? 'Creating...' : 'Generate new key'}
        </button>
      </div>

      <div style={{ marginTop: '32px' }}>
        <h3 style={{ fontSize: '0.9375rem', marginBottom: '12px' }}>Quick start — Send your first event</h3>
        <pre className="onboarding-code">{`curl -X POST ${API_BASE}/ingest/events \\
  ...
  -d '{
    ...
    "summary": "Test event from my agent",
    "outcome": "success",
    "riskScore": 10,
    "actor": "My Copilot"
  }'`}</pre>
