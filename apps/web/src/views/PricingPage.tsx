...
export function PricingPage() {
  ...
  const handleStartTrial = async () => {
    ...
    const token = localStorage.getItem('cfr_token')
    if (!token) {
      navigate('/app') // Will show login page
      return
    }
    // Try to create Stripe checkout
    try {
      ...
      if (data.fallback || data.error) {
        // Stripe not configured — just go to app
        navigate('/app')
      }
    } catch {
      navigate('/app')
      ...
      <section className="landing-section">
        <div className="landing-pricing-grid">
          ...
          <div className="landing-pricing-card featured">
            ...
            <button type="button" className="landing-cta" onClick={handleStartTrial}>Start free trial</button>
          </div>
          <div className="landing-pricing-card">
            <h3>Enterprise</h3>
